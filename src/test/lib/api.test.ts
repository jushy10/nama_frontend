...
describe('getEma', () => {
  ...
  it('requests the default 9/20/50/200 periods over the chart range', async () => {
    let requestedUrl = ''
    const body = {
      symbol: 'AAPL',
      timeframe: '1Day',
      lines: [
        ...
        { period: 20, count: 1, latest: 195.4, points: [] },
    ...
    expect(result.lines.map((l) => l.period)).toEqual([9, 20, 50, 200])
  })
